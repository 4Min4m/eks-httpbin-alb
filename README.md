...
Inbound HTTP (port 80) from the ALB (<alb-security-group>) for application traffic.
...
Tools: Terraform, Helm CLI, AWS CLI, kubectl.
AWS CLI Configuration: Configure credentials with permissions for EKS, EC2, ELB, and IAM.
Repository: Clone the project repository:git clone <repository-url>
...
Configure kubectl:aws eks update-kubeconfig --region us-east-1 --name <cluster-name>
...
Verify Deployment:
Check cluster and application:kubectl get nodes
kubectl get pods -n httpbin
kubectl get ingress -n httpbin -w


Check ALB Controller:kubectl get pods -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller


Retrieve ALB URLs:kubectl get ingress httpbin-public -n httpbin -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'
...
Public Endpoint:
Test /get from outside the VPC:curl http://<public-alb-address>/get
curl http://<public-alb-address>/status/200
...
Connect via Session Manager:aws ssm start-session --target <instance-id> --region us-east-1


Install curl and test /post:sudo yum install curl -y
...
Alternatively, test from a pod:kubectl run test --image=curlimages/curl -n httpbin --rm -it -- curl -X POST http://<private-alb-address>/post -d 'test'
...
Verify cleanup:aws elbv2 describe-load-balancers --names <private-alb-name> <public-alb-name> --region us-east-1 || echo "ALBs deleted"
...
Check controller logs: kubectl logs -n kube-system -l app.kubernetes.io/name=aws-load-balancer-controller --since=10m | grep -E 'httpbin-private|httpbin-public|error|failed'
Verify IAM role permissions: aws iam get-policy --policy-arn arn:aws:iam::<account-id>:policy/<alb-policy>
Ensure subnet tags: aws ec2 describe-subnets --subnet-ids <public-subnet-1> <public-subnet-2> <private-subnet-1> <private-subnet-2> --region us-east-1


Private ALB Accessible Externally:
Verify ALB scheme: aws elbv2 describe-load-balancers --names <private-alb-name> --region us-east-1 --query 'LoadBalancers[*].Scheme'
Check Ingress annotations: kubectl get ingress httpbin-private -n httpbin -o yaml


Pod Access Issues:
Check pod events: kubectl get events -n httpbin --sort-by='.metadata.creationTimestamp'
Verify ALB target health: aws elbv2 describe-target-health --target-group-arn <target-group-arn> --region us-east-1

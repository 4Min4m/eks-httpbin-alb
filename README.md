...
The project uses AWS services (EKS, VPC, ALB) and IaC to automate deployment, ensuring consistency and scalability. All placeholders (e.g., `<cluster-name>`, `<vpc-id>`) should be replaced with actual values from Terraform outputs or AWS console queries after deployment.
...
#### EKS Cluster (eks.tf)
Deploys an EKS cluster with worker nodes in private subnets, using `t3.medium` instances for cost efficiency and performance.

#### ALB Controller (alb.tf)
Installs the AWS Load Balancer Controller via Helm (`aws-load-balancer-controller`, version v2.13.4) in the `kube-system` namespace.  
Creates an IAM role with a comprehensive policy including permissions like `elasticloadbalancing:DescribeListenerAttributes`.

### Kubernetes Configuration
Deploys the HTTPBin application in the `httpbin` namespace with:  
...
Manifests are in `k8s/` (`namespace.yaml`, `deployment.yaml`, `service.yaml`, `ingress-public.yaml`, `ingress-private.yaml`).

## Key Implementation Challenges and Resolutions

### Duplicate IAM Role/Policy
***Issue:*** `EntityAlreadyExists` errors for IAM role and policy due to duplicates in `eks.tf` and `alb.tf`.  
***Resolution:*** Consolidated definitions in `alb.tf`, removed duplicates from `eks.tf`, and cleaned the Terraform state:  
```
terraform state rm aws_iam_role.alb_controller
...
***Resolution:*** Verified subnet annotations and tags on private subnets.

### Subnet Tags
***Issue:*** Missing `kubernetes.io/role/elb` and `kubernetes.io/role/internal-elb` tags on subnets.  
***Resolution:*** Added tags to `aws_subnet` resources in `vpc.tf`.

## Security and Policy Measures

### IAM Policies
* Created an IAM role with a policy granting necessary permissions for ALB management (e.g., `elasticloadbalancing:*`, `ec2:CreateTags`).  
...
* Configured security groups to allow:  
  ...
  2. Inbound HTTP (port 80) from the ALB for application traffic.  
...
4. **Configure AWS Auth for Console Access:**  
   ...
   a. Download the `aws-auth` ConfigMap:  
      ```
      kubectl get configmap aws-auth -n kube-system -o yaml > aws-auth.yaml
      ```

   b. Edit `aws-auth.yaml`:  
      Open `aws-auth.yaml` in an editor and add your IAM user under `mapUsers`:  
      ```
      apiVersion: v1
      kind: ConfigMap
      metadata:
        ...
        mapUsers: |
          - userarn: arn:aws:iam::<account-id>:user/<your-username>
            ...
            groups:
              - system:masters
      ```  
      Replace `<account-id>` with your AWS account ID and `<your-username>` with your IAM user name. Ensure `<node-role>` matches the EKS node group role ARN (from `eks.tf` or Terraform outputs).

   c. Apply the Updated ConfigMap:  
      ```
      kubectl apply -f aws-auth.yaml
      ```

5. **Apply Kubernetes Manifests** (first namespace, then others):  
   ```
   kubectl apply -f ../k8s/
   ```

6. **Check Cluster and Application:**  
   ```
   kubectl get nodes
...
8. **Retrieve ALB URLs:**  
   ...
   ```  
   **Note:** Wait 1-2 minutes after retrieving the URLs for the ALBs to become active before testing with `curl`.
...
### Private Endpoint
1. Launch an EC2 instance in a private subnet with the `AmazonSSMManagedInstanceCore` IAM role.  
2. Connect via Session Manager:  
   ```
   aws ssm start-session --target <instance-id> --region <your-region>
   ```  
3. Install curl and test `/post`:  
   ```
...
1. **Delete Kubernetes Resources:**  
   ...
   kubectl delete namespace httpbin  # Optional: Clean up the namespace
   kubectl get ingress -n httpbin  # Wait for deletion (may return not found)
   ```
...
└── k8s/
    ├── namespace.yaml       # HTTPBin namespace
